...
def generate_launch_description():
    ...
    imrt_teleop_ros2_control_node = Node(
        name='imrt_teleop',
        package="imrt_teleop",
        executable="imrt_teleop",
        remappings=[('/cmd_vel','/diff_cont/cmd_vel_unstamped')],
    )  

    teleop_stamper_node = Node(
        name='twist_stamper',
    ...
    text_to_speach_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory('imrt_speech'), 'launch', 'speech.launch.py')
        )
    )
